...
export const PDFCore: React.FC<{
  ...
  const [fileUrl, setFileUrl] = useState<string | null>(needDownload ? null : src);
  ...
  return (
    <div className="w-full m-auto mb-1em">
      ...
      >
        ...
        {loadingProgress === 0 ? null : <span>&nbsp;{Math.round(loadingProgress * 100)}%</span>}
      </div>
      <div
    ...
    (() => {
      ...
      if (children) {
        (Array.isArray(children) ? children : [children])
          .filter((node) => node?.props?.originalType === 'a' && node?.props?.href)
          .forEach((node) => {
            list.push({
              src: node.props.href,
              needDownload,
